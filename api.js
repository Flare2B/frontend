  ...
  $.ajax({
      ...
      success: function(data){
        for (let i = 0; i < data.length; i++) { 
          employeeCounter += 1;
          $('#table').append("<tr><td>" + data[i]["firstName"] + "</td><td>"+ data[i]["lastName"] + "</td><td>" + data[i]["email"] + "</td><td>"+ data[i]["phoneNumber"] + "</td><td><button class=\"btn btn-warning btn-delete\" id=\"" + data[i]["employeeId"] + "\">REMOVE</button></tr>");    
        } 
      },
...
function deleteEmployee(){
  $(".btn-delete").click( function() {   
    var employeeId =  $('.btn-delete').attr("employeeId");
    $.ajax({
